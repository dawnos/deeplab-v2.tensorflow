...
def _conv2d_group(inputs, repetitions, filters, pool_stride, dilation_rate, scope_id, reuse=False):
  net = slim.repeat(inputs, repetitions, slim.conv2d,
                    filters, 3, rate=dilation_rate, scope=("conv%d" % scope_id),
                    variables_collections=["pretrained"], outputs_collections=["features"], reuse=reuse)
  net = _pad(net, 1, name=("pad%d" % scope_id))
  net = slim.max_pool2d(net, 3, stride=pool_stride, scope=('pool%d' % scope_id))
  return net


def _branch(inputs, classes, scope_id, hole, branch_id):
  with tf.variable_scope('branch_%d' % branch_id):
...
def deeplab_vgg16(inputs, classes, weight_decay=0.0005, scope='deeplab', reuse=False, branch_id=0):
  with tf.variable_scope(scope):
    with slim.arg_scope([slim.conv2d], weights_regularizer=tf.contrib.layers.l2_regularizer(weight_decay)):
      net = _conv2d_group(inputs, 2,  64, pool_stride=2, dilation_rate=1, scope_id=1, reuse=reuse)
      net = _conv2d_group(   net, 2, 128, pool_stride=2, dilation_rate=1, scope_id=2, reuse=reuse)
      net = _conv2d_group(   net, 3, 256, pool_stride=2, dilation_rate=1, scope_id=3, reuse=reuse)
      net = _conv2d_group(   net, 3, 512, pool_stride=1, dilation_rate=1, scope_id=4, reuse=reuse)
      net = _conv2d_group(   net, 3, 512, pool_stride=1, dilation_rate=2, scope_id=5, reuse=reuse)

      net_1 = _branch(net, classes, 1,  6, branch_id)
      net_2 = _branch(net, classes, 2, 12, branch_id)
      net_3 = _branch(net, classes, 3, 18, branch_id)
      net_4 = _branch(net, classes, 4, 24, branch_id)
